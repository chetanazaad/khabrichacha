...
def _build_status_bar():
    with ui.row().classes("w-full status-bar items-center gap-6 px-4 py-2"):
        ...
        with ui.row().classes("items-center gap-2"):
            ...
            ui.label("Untitled").classes("text-xs font-semibold text-indigo-300")
        with ui.row().classes("items-center gap-2"):
            ui.label("\U0001f916 Model:").classes("text-xs text-gray-400")
            ui.label("None").classes("text-xs font-semibold text-indigo-300")
        with ui.row().classes("items-center gap-2 ml-auto"):
            ui.label("\U0001f7e2 Status:").classes("text-xs text-gray-400")
            ui.label("Ready").classes("status-badge status-ready")
...
def _build_mission_panel():
    with ui.column().classes("flex-1 panel gap-3"):
        ...
        goal_input = ui.textarea("Mission Objective").classes("w-full").props(
        ...
        with ui.row().classes("w-full gap-4 mt-1"):
            ...
            sources_input = ui.number(
                label="Max Sources",
                value=5, min=1, max=20,
            ).classes("w-28")

        with ui.row().classes("w-full gap-2 mt-1"):
            ui.button("Run", on_click=lambda: run_research(
                goal_input.value, model_select.value, depth_select.value, int(sources_input.value),
            )).classes("control-btn")
...
def _build_progress_header():
    with ui.column().classes("panel gap-2"):
        ...
        with ui.row().classes("w-full items-center justify-between"):
            ui.label("Step 0 of 0").classes("text-xs text-gray-400")
    ...
    with ui.element("div").classes("panel flex-1 flex flex-col"):
        ...
        with ui.tab_panels(tabs, value=tab_results).classes("w-full flex-1"):
            with ui.tab_panel(tab_results):
                ui.markdown("_No results yet. Run a research mission to see output._").classes("results-panel")

            with ui.tab_panel(tab_refs):
                ui.markdown("_No references collected._").classes("results-panel text-sm")

            with ui.tab_panel(tab_downloads):
                ui.markdown("_No downloads available._").classes("results-panel text-sm")

            with ui.tab_panel(tab_logs):
                ui.log().classes("w-full log-view h-64")
